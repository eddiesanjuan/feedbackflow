...
export class StructuredMarkdownBuilder {
  ...
  private buildSingleItem(
    ...
    const id = this.formatItemId(index);
    ...
    for (const ssIndex of item.screenshotIndices) {
      const ssId = this.formatItemId(ssIndex);
      lines.push(
        `![${id}](${options.screenshotDir}/${ssId.toLowerCase()}.png)`,
      );
    }
  ...
  private buildFooter(options: MarkdownBuildOptions): string {
    ...
    return `*Generated by [FeedbackFlow](https://github.com/eddiesanjuan/feedbackflow)${model}*\n`;
  }
  ...
  private estimateItemTimestamp(item: AIFeedbackItem, session: Session): string {
    ...
    if (matchingSegment) {
      const relativeMs = matchingSegment.timestamp - session.startTime;
